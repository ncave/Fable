...
module MapTree =
        ...
        | Some m2 ->
            ...
            | :? MapTreeNode<'Key, 'Value> as mn ->
                if c < 0 then
                    ...
                    MapTreeNode(k, v, mn.Left, mn.Right, mn.Height) :> MapTreeLeaf<'Key, 'Value>
                    |> Some
                else
                    rebalance mn.Left mn.Key mn.Value (add comparer k v mn.Right)
            | _ ->
                if c < 0 then
                    MapTreeNode(k, v, empty, m, 2) :> MapTreeLeaf<'Key, 'Value> |> Some
                elif c = 0 then
                    MapTreeLeaf(k, v) |> Some
                else
    ...
    let foldSectionOpt
        ...
        lo
        hi
        (f: OptimizedClosures.FSharpFunc<_, _, _, 'a>)
        (m: MapTree<'Key, 'Value>)
        x
        =
    ...
    let current i =
        if i.started then
            match i.stack with
            | [] -> alreadyFinished ()
            | None :: _ -> failwith "Please report error: Map iterator, unexpected stack for current"
            | Some m :: _ ->
                match m with
    ...
    let rec moveNext i =
        if i.started then
            match i.stack with
            | [] -> false
            | None :: rest -> failwith "Please report error: Map iterator, unexpected stack for moveNext"
            | Some m :: rest ->
                match m with
                | :? MapTreeNode<'Key, 'Value> ->
...
type Map<[<EqualityConditionalOn>] 'Key, [<EqualityConditionalOn; ComparisonConditionalOn>] 'Value when 'Key: comparison>
    ...
    member m.Exists predicate = MapTree.exists predicate tree

    member m.Filter predicate =
        new Map<'Key, 'Value>(comparer, MapTree.filter comparer predicate tree)

    member m.ForAll predicate = MapTree.forall predicate tree
...
let iterate action (table: Map<_, _>) = table.Iterate action
...
// [<CompiledName("Keys")>]
let keys (table: Map<'K, 'V>) : ICollection<'K> = table.Keys

// [<CompiledName("Values")>]
let values (table: Map<'K, 'V>) : ICollection<'V> = table.Values
