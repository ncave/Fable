...
type StringLiteral = | StringLiteral of value: string * loc: SourceLocation option
...
type SwitchCase = | SwitchCase of test: Expression option * consequent: Statement array * loc: SourceLocation option
...
type ExportSpecifier = | ExportSpecifier of local: Identifier * exported: Identifier
...
type FunctionTypeParam = | FunctionTypeParam of name: Identifier * typeAnnotation: TypeAnnotation * isOptional: bool
...
module Helpers =
    ...
    type Identifier with
        ...
        static member identifier(name, ?loc) : Identifier = Identifier(name, loc = loc)
    ...
    type Statement with
        ...
        static member ifStatement(test, consequent, ?alternate, ?loc) : Statement =
        ...
        static member variableDeclaration(kind, declarations, ?loc) : Statement =
            Declaration.variableDeclaration (kind, declarations, ?loc = loc) |> Declaration
    ...
    type TypeParameter with
        ...
        static member typeParameter(name, ?bound, ?``default``) = TypeParameter(name, bound, ``default``)
