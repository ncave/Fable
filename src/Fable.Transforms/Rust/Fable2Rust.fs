...
*)
    ...
    let transformCallArgs (com: IRustCompiler) ctx hasSpread args (argTypes: Fable.Type list) =
        ...
        | args ->
            if ctx.Typegen.TakingOwnership then
                args |> List.map (fun e -> transformLeaveContextByValue com ctx None None e)
            else
    ...
    let transformExprMaybeUnwrapRef (com: IRustCompiler) ctx fableExpr =
        let expr = com.TransformAsExpr(ctx, fableExpr)
        let name = tryGetIdent fableExpr |> Option.defaultValue ""
        let isRef = ctx.ScopedSymbols |> Map.tryFind name |> Option.map (fun s -> s.IsRef) |> Option.defaultValue false
        let expr =
            if shouldBeRefCountWrapped com fableExpr.Type
            then mkDerefExpr expr
            else expr
        if isRef then mkDerefExpr expr |> mkParenExpr else expr
        ...
        | _ ->
            let callee =
                ...
                | Some callMemberInfo ->
                    ...
                    mkFullNamePathExpr callMemberInfo.FullName None
                | _ ->
    ...
    let transformTest (com: IRustCompiler) ctx range kind (expr: Fable.Expr): Rust.Expr =
        ...
        | Fable.UnionCaseTest tag ->
            ...
            | Fable.DeclaredType(entRef, genArgs) ->
                ...
                let pat = mkTupleStructPat path fields
                let expr = com.TransformAsExpr(ctx, expr)
                               |>  prepareRefForPatternMatch com ctx expr.Type (tryGetIdent expr |> Option.defaultValue "")
                mkLetExpr pat expr
            | _ ->
                failwith "Should not happen"
...
*)
    let rec transformAsExpr (com: IRustCompiler) ctx (fableExpr: Fable.Expr): Rust.Expr =
        ...
        | Fable.Extended(Fable.Throw(Fable.Value(Fable.StringConstant msg, _), _), _) ->
